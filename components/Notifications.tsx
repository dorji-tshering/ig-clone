...
const Notifications = ({onClose, onModal=false}: {onClose: () => void, onModal?: boolean}) => {
    ...
    return (
        <div className={`md:max-w-[500px] ${!onModal && 'md:border md:rounded-lg'} `}>
            {/* header section */}
            <section className={`h-[50px] px-5 border-b sticky ${onModal ? 'top-0':'top-[53px] md:rounded-tr-lg md:rounded-tl-lg'}
                bg-white`}>
            ...
            <section className='py-5'>
                {
                    notis ? (
                        notis.map(noti => (
                            <div key={noti.username} onClick={() => router.push('/username')} className='flex items-center px-5 py-2 cursor-pointer'>
